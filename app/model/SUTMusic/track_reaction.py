...
from db.internal_db.SUTMusic.track_reaction_internal_db import Internal_DB_TrackReaction
...
_in = Internal_DB_TrackReaction()
track_reaction_cache = AutoExpiringDict(ttl_seconds=3600, cleanup_interval=300, max_keys=10000)
...
def cache_result(prefix: Optional[str] = None, extra_key: Optional[Callable[[tuple, dict], tuple]] = None):
    def decorator(func):
        ...
        async def wrapper(self, *args, **kwargs):
            ...
            cached = await track_reaction_cache.get(key)
            if cached is not None:
                return cached

            result = await func(self, *args, **kwargs)
            if result is not None:
                await track_reaction_cache.set(key, result)
            return result
        return wrapper
    return decorator
        ...
        async def wrapper(self, *args, **kwargs):
            ...
            if result is None or (isinstance(result, Result) and result.success):
                try:
                    ...
                    extra = extra_key(args, kwargs) if extra_key else ()
                    key = build_cache_key(self, prefix, args, kwargs, extra)
                    await track_reaction_cache.set(key, value)
                except Exception as e:
                    print(f"[⚠️ cache_update_dynamic] Failed to cache at track reaction func: {func.__name__} : {e}")
            return result
        return wrapper
    return decorator
...
class TrackReaction:
    ...
    def __init__(self, entry_id: Optional[Union[int, str]] = None) -> None:
        """
        High-level business-logic wrapper abstracting a track reaction lifecycle.
        """
        self.entry_id = int(entry_id) if entry_id is not None else None

    @classmethod
    async def get_by_id(cls, entry_id: Union[int, str]) -> Optional["TrackReaction"]:
        obj = await _in.get_reaction_by_id(int(entry_id))
        if obj:
            return TrackReaction(obj.entry_id)
        return None
    ...
    async def search_reactions(
        ...
        if objs:
            return [TrackReaction(obj.entry_id) for obj in objs]
        return None
    ...
    async def create(
        ...
        track_reaction = await cls.search_reactions(conditions={"reaction_id": ("=", reaction_id),
                                                                "user_id": ("=", user_id),
                                                                "track_id": ("=", track_id)},
                                                    limit=1)
        if track_reaction and len(track_reaction) > 0:
            return Result(True, "create", f"Track reaction already exists: {track_reaction}", track_reaction[0])
        ...
        if result.success:
            ...
            result.data = TrackReaction(entry_id)
        return result
...
async def main():
    ...
    # -------------------------------------------------------------------------
    # print("--- Test 1: Creating Track Reaction Records ---")
    #
    # sample_track_id = 7
    # sample_user_id = 615917499502
    # sample_reaction = 1
    # sample_sentiment = "like"
    # sample_chat_id = 6057721454
    # sample_message_id = 452
    # sample_genre_id = 2
    #
    # creation_result = await TrackReaction.create(
    ...
    track_reaction = await TrackReaction.search_reactions(conditions={"reaction_id": ("=", 75),
                                                            "user_id": ("=", 119787983407),
                                                            "track_id": ("=", 101)}, limit=1)
    print(track_reaction)

    # if not creation_result.success:
    #     print(f"Stopping tests. Insertion execution failed: {creation_result.message}")
    #     return
    #
    # reaction_instance: TrackReaction = creation_result.data
    ...
    # boundary_test = await TrackReaction.create(
    #     track_id=sample_track_id, user_id=sample_user_id, reaction="👍", sentiment="invalid_sentiment"
    # )
    ...
    #
    # fetched_reaction = await TrackReaction.get_by_id(target_id)
    # if fetched_reaction:
    ...
    # }
    #
    # search_results = await TrackReaction.search_reactions(
    ...
    #
    # post_deletion_lookup = await TrackReaction.get_by_id(target_id)
