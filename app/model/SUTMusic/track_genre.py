...
from db.internal_db.SUTMusic.track_genre_internal_db import Internal_DB_TrackGenre
from utils.result import Result
from utils.schedule.dict_helper import AutoExpiringDict

# Field configurations based closely on track_genres architectural relational layouts
...
_in = Internal_DB_TrackGenre()

# Internal structural caching dictionary tracking transient parameter values to minimize database load
track_genre_cache = AutoExpiringDict(ttl_seconds=3600, cleanup_interval=300, max_keys=10000)
...
def cache_result(prefix: Optional[str] = None, extra_key: Optional[Callable[[tuple, dict], tuple]] = None):
    def decorator(func):
        ...
        async def wrapper(self, *args, **kwargs):
            ...
            cached = await track_genre_cache.get(key)
            if cached is not None:
                return cached

            result = await func(self, *args, **kwargs)
            if result is not None:
                await track_genre_cache.set(key, result)
            return result
        ...
        async def wrapper(self, *args, **kwargs):
            ...
            if result is None or (isinstance(result, Result) and result.success):
                try:
                    ...
                    extra = extra_key(args, kwargs) if extra_key else ()
                    key = build_cache_key(self, prefix, args, kwargs, extra)
                    await track_genre_cache.set(key, value)
                except Exception as e:
                    print(f"[⚠️ cache_update_dynamic] Failed to cache at track genre func: {func.__name__} : {e}")
            return result
...
class TrackGenre:
    ...
    def __init__(self, entry_id: Optional[Union[int, str]] = None) -> None:
        """
        High-level business-logic wrapper abstracting a track genre relationship lifecycle.
        """
        self.entry_id = int(entry_id) if entry_id is not None else None

    @classmethod
    async def get_by_id(cls, entry_id: Union[int, str]) -> Optional["TrackGenre"]:
        obj = await _in.get_genre_by_id(int(entry_id))
        if obj:
            return TrackGenre(obj.entry_id)
        return None
    ...
    async def search_genres(
        ...
        if objs:
            return [TrackGenre(obj.entry_id) for obj in objs]
        return None

    @classmethod
    async def create(cls, track_id: int, genre_id: int, confidence: float = 1.0, source: str = "llm") -> Result:
        ...
        if result.success:
            ...
            result.data = TrackGenre(entry_id)
        return result
...
async def main():
    ...
    # -------------------------------------------------------------------------
    print("--- Test 1: Creating Track Genre Association Records ---")
    ...
    creation_result = await TrackGenre.create(
    ...
    genre_instance: TrackGenre = creation_result.data
    ...
    boundary_test = await TrackGenre.create(track_id=sample_track_id, genre_id=sample_genre_id, confidence=1.5)
    ...
    fetched_genre = await TrackGenre.get_by_id(target_id)
    if fetched_genre:
    ...
    search_results = await TrackGenre.search_genres(
    ...
    post_deletion_lookup = await TrackGenre.get_by_id(target_id)
