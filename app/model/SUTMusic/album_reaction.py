...
from db.internal_db.SUTMusic.album_reaction_internal_db import Internal_DB_AlbumReaction
...
_in = Internal_DB_AlbumReaction()
album_reaction_cache = AutoExpiringDict(ttl_seconds=3600, cleanup_interval=300, max_keys=10000)
...
def cache_result(prefix: Optional[str] = None, extra_key: Optional[Callable[[tuple, dict], tuple]] = None):
    def decorator(func):
        ...
        async def wrapper(self, *args, **kwargs):
            ...
            cached = await album_reaction_cache.get(key)
            if cached is not None:
                return cached

            result = await func(self, *args, **kwargs)
            if result is not None:
                await album_reaction_cache.set(key, result)
            return result
        return wrapper
    return decorator
        ...
        async def wrapper(self, *args, **kwargs):
            ...
            if result is None or (isinstance(result, Result) and result.success):
                try:
                    ...
                    extra = extra_key(args, kwargs) if extra_key else ()
                    key = build_cache_key(self, prefix, args, kwargs, extra)
                    await album_reaction_cache.set(key, value)
                except Exception as e:
                    print(f"[⚠️ cache_update_dynamic] Failed to cache at album reaction func: {func.__name__} : {e}")
            return result
        return wrapper
    return decorator
...
class AlbumReaction:
    ...
    def __init__(self, entry_id: Optional[Union[int, str]] = None) -> None:
        """
        High-level business-logic wrapper abstracting an album reaction lifecycle.
        """
        self.entry_id = int(entry_id) if entry_id is not None else None

    @classmethod
    async def get_by_id(cls, entry_id: Union[int, str]) -> Optional["AlbumReaction"]:
        obj = await _in.get_reaction_by_id(int(entry_id))
        if obj:
            return AlbumReaction(obj.entry_id)
        return None
    ...
    async def search_reactions(
        ...
        if objs:
            return [AlbumReaction(obj.entry_id) for obj in objs]
        return None
    ...
    async def create(
        ...
        album_reaction = await cls.search_reactions(conditions={"reaction_id": ("=", reaction_id),
                                                                "user_id": ("=", user_id),
                                                                "album_id": ("=", album_id)},
                                                    limit=1)
        if album_reaction and len(album_reaction) > 0:
            return Result(True, "create", f"Track reaction already exists: {album_reaction}", album_reaction[0])
        ...
        if result.success:
            ...
            result.data = AlbumReaction(entry_id)
        return result
...
async def main():
    ...
    # -------------------------------------------------------------------------
    print(await AlbumReaction.search_reactions(conditions={"reaction_id": ("="), "user_id": ("="), "album_id": ("=")}))
    # print("--- Test 1: Creating Album Reaction Records ---")
    #
    # sample_album_id = 4
    ...
    #
    # creation_result = await AlbumReaction.create(
    ...
    #     return
    #
    # reaction_instance: AlbumReaction = creation_result.data
    ...
    # boundary_test = await AlbumReaction.create(
    #     album_id=sample_album_id, user_id=sample_user_id, reaction="👍", sentiment="invalid_sentiment"
    # )
    ...
    #
    # fetched_reaction = await AlbumReaction.get_by_id(target_id)
    # if fetched_reaction:
    ...
    # }
    #
    # search_results = await AlbumReaction.search_reactions(
    ...
    #
    # post_deletion_lookup = await AlbumReaction.get_by_id(target_id)
