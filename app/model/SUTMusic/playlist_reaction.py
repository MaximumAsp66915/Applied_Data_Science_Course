...
from db.internal_db.SUTMusic.playlist_reaction_internal_db import Internal_DB_PlaylistReaction
...
_in = Internal_DB_PlaylistReaction()
playlist_reaction_cache = AutoExpiringDict(ttl_seconds=3600, cleanup_interval=300, max_keys=10000)
...
def cache_result(prefix: Optional[str] = None, extra_key: Optional[Callable[[tuple, dict], tuple]] = None):
    def decorator(func):
        ...
        async def wrapper(self, *args, **kwargs):
            ...
            cached = await playlist_reaction_cache.get(key)
            if cached is not None:
                return cached

            result = await func(self, *args, **kwargs)
            if result is not None:
                await playlist_reaction_cache.set(key, result)
            return result
        return wrapper
    return decorator
        ...
        async def wrapper(self, *args, **kwargs):
            ...
            if result is None or (isinstance(result, Result) and result.success):
                try:
                    ...
                    extra = extra_key(args, kwargs) if extra_key else ()
                    key = build_cache_key(self, prefix, args, kwargs, extra)
                    await playlist_reaction_cache.set(key, value)
                except Exception as e:
                    print(f"[⚠️ cache_update_dynamic] Failed to cache at playlist reaction func: {func.__name__} : {e}")
            return result
        return wrapper
    return decorator


class PlaylistReaction:
    _lock = asyncio.Lock()

    def __init__(self, entry_id: Optional[Union[int, str]] = None) -> None:
        """
        High-level business-logic wrapper abstracting a playlist reaction lifecycle.
        """
        self.entry_id = int(entry_id) if entry_id is not None else None

    @classmethod
    async def get_by_id(cls, entry_id: Union[int, str]) -> Optional["PlaylistReaction"]:
        obj = await _in.get_reaction_by_id(int(entry_id))
        if obj:
            return PlaylistReaction(obj.entry_id)
        return None
    ...
    async def search_reactions(
    ...
    ) -> Optional[list["PlaylistReaction"]]:
        ...
        if objs:
            return [PlaylistReaction(obj.entry_id) for obj in objs]
        return None
    ...
    async def create(
        ...
        playlist_reaction = await cls.search_reactions(conditions={"reaction_id": ("=", reaction_id),
                                                                "user_id": ("=", user_id),
                                                                "playlist_id": ("=", playlist_id)},
                                                    limit=1)
        if playlist_reaction and len(playlist_reaction) > 0:
            return Result(True, "create", f"Track reaction already exists: {playlist_reaction}", playlist_reaction[0])
        ...
        if result.success:
            ...
            result.data = PlaylistReaction(entry_id)
        return result
...
async def main():
    ...
    # -------------------------------------------------------------------------
    print("--- Test 1: Creating Playlist Reaction Records ---")
    ...
    sample_message_id = 124

    creation_result = await PlaylistReaction.create(
    ...
    reaction_instance: PlaylistReaction = creation_result.data
    ...
    boundary_test = await PlaylistReaction.create(
        playlist_id=sample_playlist_id, user_id=sample_user_id, reaction="👍", sentiment="invalid_sentiment"
    )
    ...
    fetched_reaction = await PlaylistReaction.get_by_id(target_id)
    if fetched_reaction:
    ...
    search_results = await PlaylistReaction.search_reactions(
    ...
    post_deletion_lookup = await PlaylistReaction.get_by_id(target_id)
