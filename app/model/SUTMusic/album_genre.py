...
from db.internal_db.SUTMusic.album_genre_internal_db import Internal_DB_AlbumGenre
from utils.result import Result
from utils.schedule.dict_helper import AutoExpiringDict

# Field configurations based closely on album_genres architectural relational layouts
...
_in = Internal_DB_AlbumGenre()

# Internal structural caching dictionary tracking transient parameter values to minimize database load
album_genre_cache = AutoExpiringDict(ttl_seconds=3600, cleanup_interval=300, max_keys=10000)
...
def cache_result(prefix: Optional[str] = None, extra_key: Optional[Callable[[tuple, dict], tuple]] = None):
    def decorator(func):
        ...
        async def wrapper(self, *args, **kwargs):
            ...
            cached = await album_genre_cache.get(key)
            if cached is not None:
                return cached

            result = await func(self, *args, **kwargs)
            if result is not None:
                await album_genre_cache.set(key, result)
            return result
        ...
        async def wrapper(self, *args, **kwargs):
            ...
            if result is None or (isinstance(result, Result) and result.success):
                try:
                    ...
                    extra = extra_key(args, kwargs) if extra_key else ()
                    key = build_cache_key(self, prefix, args, kwargs, extra)
                    await album_genre_cache.set(key, value)
                except Exception as e:
                    print(f"[⚠️ cache_update_dynamic] Failed to cache at album genre func: {func.__name__} : {e}")
            return result
...
class AlbumGenre:
    ...
    def __init__(self, entry_id: Optional[Union[int, str]] = None) -> None:
        """
        High-level business-logic wrapper abstracting an album genre relationship lifecycle.
        """
        self.entry_id = int(entry_id) if entry_id is not None else None

    @classmethod
    async def get_by_id(cls, entry_id: Union[int, str]) -> Optional["AlbumGenre"]:
        obj = await _in.get_genre_by_id(int(entry_id))
        if obj:
            return AlbumGenre(obj.entry_id)
        return None
    ...
    async def search_genres(
        ...
        if objs:
            return [AlbumGenre(obj.entry_id) for obj in objs]
        return None

    @classmethod
    async def create(cls, album_id: int, genre_id: int, confidence: float = 1.0, source: str = "llm") -> Result:
        ...
        if result.success:
            ...
            result.data = AlbumGenre(entry_id)
        return result
...
async def main():
    ...
    # -------------------------------------------------------------------------
    print("--- Test 1: Creating Album Genre Association Records ---")
    ...
    creation_result = await AlbumGenre.create(
    ...
    genre_instance: AlbumGenre = creation_result.data
    ...
    boundary_test = await AlbumGenre.create(album_id=sample_album_id, genre_id=sample_genre_id, confidence=1.5)
    ...
    fetched_genre = await AlbumGenre.get_by_id(target_id)
    if fetched_genre:
    ...
    search_results = await AlbumGenre.search_genres(
    ...
    post_deletion_lookup = await AlbumGenre.get_by_id(target_id)
