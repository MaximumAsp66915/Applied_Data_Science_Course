...
# Field mapping configurations based on the 'album_reactions' database schema
scalar_fields = {
    "id",
    "album_id",
    "user_id",
...
class AlbumReaction_DB:
    lock = asyncio.Lock()

    def __init__(self, db: "Internal_DB_Connection" = None, entry_id: Optional[int] = None) -> None:
        """
        Initializes the low-level album reaction database access object.
        """
    ...
    async def get_reaction_row(self, entry_id: Optional[int] = None) -> Optional[dict]:
        """
        Fetches the complete raw dictionary representation of a specific row from 'album_reactions'.
        """
        entry_id = entry_id or self.entry_id
        if entry_id is None:
            return None
        result = await self.db.get_row("album_reactions", {"id": entry_id})
        if result.success and result.data:
            return dict(result.data)
        return None

    async def get_reaction_by_id(self, entry_id: Optional[int] = None) -> Optional["AlbumReaction_DB"]:
        """
        ...
        if entry_id is None:
            return None
        result = await self.db.get_value("album_reactions", {"id": entry_id}, "id")
        if result.success and result.data:
            return AlbumReaction_DB(db=self.db, entry_id=result.data)
        return None
    ...
    async def search_reactions(
    ...
    ) -> Optional[list["AlbumReaction_DB"]]:
        """
        Searches the 'album_reactions' table using structured comparison conditions.
        """
        ...
        if result.success and result.data:
            for i in range(len(result.data)):
                result.data[i] = AlbumReaction_DB(db=self.db, entry_id=result.data[i])
            return result.data
        return None

    async def add_reaction(self, reaction_data: Dict[str, Any]) -> Result:
        """
        Executes an INSERT operation to construct a new row within the album_reactions table.
        """
        return await self.db.insert_and_return_id(
            table="album_reactions",
    ...
    async def get_parameter_from_db(self, entry_id: Optional[int], param: str) -> Result:
        ...
        return await self.db.get_value("album_reactions", {"id": entry_id}, param)
    ...
    async def update_parameter(self, entry_id: Optional[int], param: str, value: Any) -> Result:
        ...
        return await self.db.update_row(
            "album_reactions", {param: value}, {"id": entry_id}
        )
    ...
    async def delete_reaction_by_id(self, entry_id: Optional[int] = None) -> Result:
        ...
        return await self.db.delete_row("album_reactions", {"id": entry_id})

    def __repr__(self) -> str:
        if self.entry_id is None:
            text = f"""[{self.__class__.__name__} Class attribution]"""
        else:
            text = f"""[{self.__class__.__name__} Object] -> album reaction entry: {self.entry_id} (from {self.db.__class__.__name__})"""
        return text