...
2. **Clean the performer string** — strip bracketed junk (`[]`, `()`, `{}`, `<>`), promotional trailers after `•` or `@`, using a couple of surgical regexes (`_get_clean_performer`).
3. **Split into individual artists** — a cleaned performer string like `"Daft Punk ft. Pharrell Williams"` gets split on delimiters (`ft`, `feat`, `vs`, `&`, `/`, `,`, `-`, …) into a proper artist list (`_extract_artists`).
4. **Forward the file to a private backup channel.** This is the single most important reliability decision in the whole pipeline: if the *original* message ever gets deleted from the public group, the bot doesn't end up holding a dangling, unplayable `file_id`. The backup copy keeps the actual bytes safe and permanently accessible.
5. **Resolve (or create) the sender's `User` + `Chat` records.**
6. **Resolve (or create) every artist**, matching case-insensitively against what's already in the database before ever inserting a duplicate.
7. **Resolve (or create) the `Track` record itself**, matched by `(title, performer)` — if it already exists (someone re-shared the same file), the new uploader is simply appended to `uploaded_by` instead of creating a duplicate row.
8. **Kick off reaction collection** for that message, if any reactions are visible.
...
- Maps the raw emoji to a `ReactionType` — and if it's never been seen before, it's logged and auto-registered on the fly, defaulting to `"neutral"` sentiment until a human tags it properly.
...
flowchart TD
    ...
    C -- no --> B
    C -- yes --> D[🧹 Parse & clean title / performer]
    D --> E[📤 Forward file to private backup channel]
    E --> F[👤 Resolve / create sender User + Chat]
    F --> G[🎤 Resolve / create Artist rows]
    G --> H[🎵 Resolve / create Track row]
    H --> I{Reactions visible?}
    I -- yes --> J[📄 Page through reactions<br/>100 at a time]
    J --> K[👥 Resolve / create reactor User]
    K --> L[😀 Map emoji → sentiment]
