...
> 🧵 This queue isn't purely reactive anymore, either. The scraper (Ch.1) now proactively drops a job on this exact same `EnrichmentQueue` machinery the moment it creates a new track or artist — running its own small pool, in its own process, against the same underlying Last.fm/fanart.tv functions. By the time a real user's request ever reaches a freshly-scraped track, enrichment may well already be sitting in the DB, `_pending` never even coming into play.
